...
const baseFormSchema = z.object({
  ...
  owner: z.number().int().or(z.string()).pipe(z.coerce.number().int()),
  city: z
    .string()
    .min(1, {
...
function AdminEditForm({id, countries, companyData, users}) {
  ...
  const form = useForm({
    ...
    defaultValues: {
      ...
      country: companyData?.countries.id || "",
      owner: companyData?.owner_user_id || "",
      city: companyData?.city || "",
      street: companyData?.street || "",
      building: companyData?.building || "",
      contact: companyData?.contact_persons[0] ? true : false,
      personFirstName: companyData?.contact_persons[0]?.first_name || "",
      personLastName: companyData?.contact_persons[0]?.last_name || "",
      personMiddleName: companyData?.contact_persons[0]?.middle_name || "",
      personNumber: companyData?.contact_persons[0]?.phone_number || "",
      personPosition: companyData?.contact_persons[0]?.position || "",
    },
  });
  ...
  const onSubmit = async (values) => {
    ...
    setLoading(true);
    axios
      .put(import.meta.env.VITE_API_URL + "companies/" + id, values, {
        withCredentials: true,
      })
      .then((response) => {
        console.log(response.data);
        toast({
          title: "Предприятие успешно обновлено",
          description: "Теперь мы можете продолжать вести отчётность предприятия " + response.data.title,
        });
